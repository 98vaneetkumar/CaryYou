...
module.exports = {
  licenseDetailsAdd: async (req, res) => {
    try {
      ...
      if (req.files && req.files.picOfVehicle) {
        const picOfVehicleFiles = Array.isArray(req.files.picOfVehicle)
      ...
      if (req.files && req.files.picOfVehicleRegistration) {
        const picOfVehicleRegistrationFiles = Array.isArray(
      ...
      if (req.files && req.files.insurancePolicy) {
        const insurancePolicyFiles = Array.isArray(req.files.insurancePolicy)
      ...
      if (result.modifiedCount > 0) {
        return commonHelper.success(
          res,
          Response.success_msg.licenceDetailsAdd,
          response
        );
      } else {
      ...
      return commonHelper.error(
        res,
        Response.error_msg.riderData_failed,
        error.message
      );
    }
  },
  licenseDetailsUpdate: async (req, res) => {
    try {
      ...
      if (req.files?.picOfVehicleRegistration) {
        picOfVehicleRegistrationArray.push(
          ...(await uploadFile(req.files.picOfVehicleRegistration))
        );
      }
      if (req.files?.insurancePolicy) {
        insurancePolicyArray.push(
          ...(await uploadFile(req.files.insurancePolicy))
        );
      }
      ...
      const licenceUpdate = {
        ...
        ...(licenceImagePaths.length && {
          licenceImage: existingUser?.licenceDetails?.licenceImage
            ? [
                ...existingUser.licenceDetails.licenceImage,
                ...licenceImagePaths,
              ]
            : licenceImagePaths,
        }),
      ...
      const vehicleUpdate = {
        ...
        ...(picOfVehicleRegistrationArray.length && {
          picOfVehicleRegistration: existingUser?.vehicleInfo
            ?.picOfVehicleRegistration
            ? [
        ...
        ...(insurancePolicyArray.length && {
          insurancePolicy: existingUser?.vehicleInfo?.insurancePolicy
            ? [
                ...existingUser.vehicleInfo.insurancePolicy,
                ...insurancePolicyArray,
              ]
            : insurancePolicyArray,
        }),
        ...(payload.insuranceExpiryDate && {
      ...
      const result = await Models.userModel.updateOne(
        { _id: req.user._id },
        {
          ...(Object.keys(licenceUpdate).length > 0 && {
            $set: { licenceDetails: licenceUpdate },
          }),
          ...(Object.keys(vehicleUpdate).length > 0 && {
            $set: { vehicleInfo: vehicleUpdate },
          }),
        }
      );
      ...
      if (result.modifiedCount > 0) {
        return commonHelper.success(
          res,
          Response.success_msg.licenceDetailsUpdated,
          updatedUser
        );
      } else {
        return commonHelper.error(
  ...
  feedBackSend: async (req, res) => {
    try {
      ...
      return commonHelper.success(
        res,
        Response.success_msg.feedBack_send_success,
        result
      );
    } catch (error) {
